...
### D-155 — Un geste dont on ne voit pas l'effet est un geste qu'on refait

Trois remarques du bloc O-5 disaient la même chose sous trois formes, et il a
fallu les trois pour voir qu'il s'agissait d'un seul défaut de conception.

**« Personne à l'adresse n'a pas de suite. »** La cause était au fond : l'arrêt
restait `A_FAIRE` après un signalement, donc « prochain arrêt » redonnait la
**même adresse indéfiniment**. Le livreur signalait une absence et rien ne
bougeait — il ne pouvait que recommencer.

Le premier passage sans réponse **reporte** désormais l'arrêt **en fin de
tournée** : on continue, on y revient. C'est ce que fait tout livreur, et cela
laisse au client sa deuxième chance. Le deuxième passage fait repartir le colis,
comme avant (D-23).
...
heures, puis un administrateur tranche avec les deux versions — mais **le client
ne la voyait nulle part**. Le dossier s'affiche maintenant sur la commande
elle-même, avec la réponse de la boutique, le délai, puis la décision. Un
signalement dont on n'apprend jamais l'issue est un formulaire, pas un recours.

**« Tu cliques les livraisons échouées, ça ramène sur les gains, pourquoi ? »**
L'écran ne savait dire qu'une chose — « tournée terminée, voir mes gains » — et
il la disait dès que « prochain arrêt » était vide, **y compris quand des
arrêts avaient échoué**. Il distingue maintenant trois situations : pas de
tournée, tournée finie, et tournée finie **avec des échecs** — qui renvoie au
détail de la journée, pas au portefeuille.


### D-156 — Une liste dépliée n'est pas une liste

**Ton bloc O-5** : *« mes commandes, avec tous les reçus dépliés à la figure »*,
*« l'historique est déplié, laid, non cliquable »*, et *« lorsque "donner un
avis" apparaît, les trois boutons sont mal positionnés et laids »*.

Trois écrans, un même défaut : **tout était montré tout le temps.**

- **Mes commandes** affichait la frise de six étapes, le pied et les boutons de
  chaque commande. Quatre commandes faisaient un mur. Les cartes se replient sur
  l'essentiel — numéro, boutique, état, montant, jauge — et **celle qui est en
  cours reste ouverte** : c'est celle qu'on vient voir ;
- **l'historique du livreur** alignait une carte pleine largeur par course, sans
  rien à ouvrir. Il est groupé **par jour**, avec le total gagné en tête —
  l'unité dans laquelle un livreur pense — une ligne dense par course, et chaque
  ligne s'ouvre sur le détail, **calcul de la rémunération compris** ;
- **les trois boutons** étaient trois boutons de largeurs inégales qui passaient
  à la ligne. Une grille à parts égales, symbole au-dessus du mot : ils font la
  même taille quel que soit leur texte, et ils tiennent sur une ligne.

**Et le reçu s'imprime.** Le navigateur sait imprimer et propose lui-même
« Enregistrer au format PDF », sur Android comme sur iOS : on ne refait pas son
travail, on lui donne une feuille propre. La règle d'impression est la même que
la facture du web (D-78) : **tout ce qui sert à naviguer disparaît**.


### D-157 — Les filtres partent dans un panneau du bas

**Ton bloc O-1** : *« pas dense, pas assez de symboles, pas de popups, pas de
panneau qui surgit en bas ou à droite »*, et **O-2** : *« tout doit être caché
les uns derrière les autres, pour rendre très dense »*.

La recherche mobile portait ses filtres **à plat, en haut, en permanence** : un
tiers de la hauteur d'un téléphone consommé par des boutons qu'on touche une
fois par visite. Et ils n'exposaient que le mode de livraison — ni catégorie, ni
boutique, ni disponibilité, alors que le serveur rend tout cela dans ses
facettes (D-35).

L'en-tête tient maintenant sur **une ligne** : la recherche, et un bouton qui
**dit combien de filtres sont actifs**. Le reste vit dans une feuille qui monte
du bas, avec les catégories groupées par univers. Les filtres retenus restent
visibles en pastilles, chacune se retirant d'un appui — sans elles, on ne sait
plus pourquoi la liste est si courte.

Les facettes viennent de la **même requête** que la liste (`appelerComplet` rend
`data` *et* `meta`) : deux appels pour la même adresse doubleraient le travail du
serveur à chaque frappe. Et comme elles décrivent ce qui reste après le filtrage
géographique, **un filtre ne mène jamais à « aucun résultat »**.

La vignette produit est désormais **un seul composant**, partagé par l'accueil
et la recherche. Un produit qui n'a pas la même tête selon l'écran où on le
croise est ce qui donne l'impression d'une application « détachée » — c'était
ton premier mot du bloc.


### D-158 — La barre d'Android et la nôtre ne se marchent plus dessus

**Ton bloc O-3** : *« les 3 onglets naturels d'Android cassent mes 5 onglets
mécaniques »*.

Le code posait pourtant `padding-bottom: env(safe-area-inset-bottom)`, ce qui
paraît suffisant. **C'est le piège** : sur un iPhone cette marge vaut 34 pixels
et tout va bien, mais **sur un Android à trois boutons elle vaut zéro**. La
WebView n'y est pas dessinée bord à bord, donc le système ne déclare aucune
marge — et nos onglets se retrouvent posés directement sur les siens.

`max(env(safe-area-inset-bottom), 12px)` garantit une clairière dans tous les
cas : la marge du système quand elle existe, douze pixels sinon. C'est peu, et
c'est exactement ce qui sépare deux barres de deux barres collées. La même
variable sert à toutes les feuilles du bas, qui souffraient du même défaut.

**Les cinq onglets restent cinq** : c'est ta règle d'or n°10, et le « + » du
milieu en fait partie. Le problème n'était pas leur nombre mais leur position.

Ionic mesure par ailleurs la hauteur en `vh`, qui ignore la barre d'URL des
navigateurs mobiles — celle-ci recouvrait le bas de la page. `dvh` suit la
hauteur réellement visible.


### D-159 — Une aide qui parle du métier d'un autre apprend à ne pas lire l'aide

**Ton bloc O-4** : *« le support des livreurs chez le client, les questions des
Standard chez les Express »*.

C'était la même faute deux fois : **une seule liste de questions servait tout le
monde**. Un client lisait *« le client ne répond pas, que faire ? »* — une
question qu'il ne se posera jamais — et un livreur Express lisait *« puis-je
changer l'ordre de ma tournée ? »*, alors qu'il n'en a pas.

Ce n'est pas qu'inélégant. **Après deux questions hors sujet, on referme et on
écrit au support** : une aide mal ciblée coûte plus cher que pas d'aide du tout.

Les questions sont rangées par public — client, livreur commun, Express,
Standard — et l'écran ne montre que celles du rôle connecté, et de son mode. Il
le dit d'ailleurs en tête, pour qu'on sache qu'on est au bon endroit.
